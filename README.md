...
`Study-I/analysis.ipynb` has outputs for our original experiment run, a new set of data can be generated using the `Study-I/experiment.py` script. The analysis notebook can be run with the new data by changing the `data/results.csv` filepath in the beginning of the notebook to the resulting `results_YYYYMMDD_HHMMSS.csv` file

To do this, you need an OpenRouter API key, and a local installation of Ollama with `gpt-oss:20b` and `qwen3:14b`.
